        ...
        for example in items:
            ...
            for i in range(len(positive_responses)):
                ...
                if data_args.type_negative == 'both':
                    if i >= len(example[data_args.type_negative_extra]):
                        data_with_less_negs+=1
                        # print('found less traininstance type extra at ', i, len(example[data_args.type_negative_extra]))
                        continue
                    ...
                    all_segment_ids.append(segment_ids)
                    nsp_labels.append(1)
                    # print('extin negative_response', negative_response)


            #add extra negative responses if present
            if data_args.type_negative == 'both':
                max_num_neg = min(len(example[data_args.type_negative_extra]), data_args.type_negative_extra_max)
                for i in range(len(positive_responses), max_num_neg):
                    if i >= len(example[data_args.type_negative_extra]):
                        # print('found less traininstance type extra at ', i, len(example[data_args.type_negative_extra]))
                        continue
                    negative_response = example[data_args.type_negative_extra][i].lower()
                    ...
                    all_segment_ids.append(segment_ids)
                    nsp_labels.append(1)
                    # print('extext negative_response', negative_response)
        # import pdb;pdb.set_trace()
