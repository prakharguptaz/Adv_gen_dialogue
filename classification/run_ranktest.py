...
def main():
    ...
    def preprocess_function_test(examples):
        ...
        for example in items:
            ...
            for i in range(len(positive_responses)):
                ...
                for j in range(len(negative_responses)):
                    negative_response = negative_responses[j]
                    tokens_b = negative_response
                    tokens_b = tokenizer.tokenize(speaker2 + ' ' + tokens_b, max_length=data_args.max_seq_length-5)
                    tokens, segment_ids = get_string_text(tokens_a, tokens_b)
                    all_texts.append(tokens)
    ...
    if training_args.do_eval:
        ...
        for eval_dataset, task in zip(eval_datasets, tasks):
            ...
            output_eval_file = os.path.join(training_args.output_dir, f"rankeval_results_{task}.txt")
...
####### For test
    ...
    if training_args.do_predict:
        ...
        for test_dataset, task in zip(test_datasets, tasks):
            ...
            output_testmetric_file = os.path.join(training_args.output_dir, f"ranktest_metric_results_{task}.txt")
            # if trainer.is_world_process_zero():
            ...
            if 0:
                ...
                output_test_file = os.path.join(training_args.output_dir, f"ranktest_outputs_{task}.csv")
    ...
    if training_args.do_predict:
        ...
        for test_dataset, task in zip(test_datasets, tasks):
            ...
            test_result = trainer.evaluate(eval_dataset=test_dataset)
            print('test ', test_result)
            output_testmetric_file = os.path.join(training_args.output_dir, f"rankrandom_test_metric_results_{task}.txt")
            # if trainer.is_world_process_zero():
            with open(output_testmetric_file, "w") as writer:
                logger.info(f"***** Random Test results {task} *****")
                for key, value in test_result.items():
                    logger.info(f"  {key} = {value}")
                    writer.write(f"{key} = {value}\n")
            if 0:
                ...
                scores =  predictions
                predictions = np.squeeze(predictions) if is_regression else np.argmax(predictions, axis=1)
                output_test_file = os.path.join(training_args.output_dir, f"rankrandom_test_outputs_{task}.csv")
                csvwriter = csv.writer(open(output_test_file,'w'), delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
...
#python run_dialclassifier.py --model_name_or_path tmp/test-mlm/ --train_file /home/ubuntu/Code/negaug/dataset/train_e1.json --validation_file /home/ubuntu/Code/negaug/dataset/dev_e1.json --test_file /home/ubuntu/Code/negaug/dataset/test.json --output_dir tmp/test-e1_logs --per_device_train_batch_size 54 --per_device_eval_batch_size 12  --type_negative_test adversarial_negative_responses --overwrite_cache --type_negative both --type_negative_extra  adv_gen_neg_responses_t1  --do_eval --do_predict --do_train --overwrite_output_dir --evaluation_strategy steps --eval_steps 200 --save_steps 200
