...
def randomly_mask_document(
  ...
  def mask_acceptable(masked_spans):
    ...
    len_doc = len(doc)
    # if len(doc)==2 and type(doc) == dict and 'response' in doc:
    if type(doc) == dict and 'response' in doc:
      len_doc = len(doc['response'])

    if ensure_valid_bounds_in_spans and not masked_spans_bounds_valid(masked_spans, len_doc):
...
if __name__ == '__main__':
  ...
  if args.mask_arg0 is None:
    # masker = mask_type(mask_document_p=0, mask_document_p=0)
    #modified
    masker = mask_type(p=0.15, mask_document_p=0, mask_paragraph_p=0)
  else:
...
#python create_ilm_examples.py test data/char_masks/dailydialog --seed 0 --data_name dailydialog --data_split test --mask_cls ilm.mask.hierarchical_dailydialog.MaskHierarchical --data_dir data/dailydialog
#python create_ilm_examples.py train data/char_masks/personachat --seed 0 --data_name personachat --data_split train --mask_cls ilm.mask.hierarchical_dailydialog.MaskHierarchical
